...
- **`OutputContent.overflow` becomes required in `openapi.yaml`**, matching the schema the
  file implements.
...
- **`frontend/tsconfig.json`, `Makefile`, root `Makefile`**: the generated copy into
  `frontend/src`.
- **`openapi/openapi.yaml`**: one line, `required: [max, overflow]`.
- **Not affected**: the server's behaviour. The `openapi.yaml` fix corrects what the schema
  admits, and the endpoint already enforces the standard.
