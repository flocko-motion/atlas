## 1. Bump ranke-go (v0.4.0, v0.5.0 wire codec, v0.6.0 declared header, v0.10.0–v0.12.0 constraints)
...
- [x] 1.4 v0.10.0 adds `Constraints` and makes `NewContribution` variadic, so the existing call compiles unchanged and inherits the strictest default — which is what a client contribution should get.
...
- [x] 4.6 Map library failures to the sentinels; absent and out-of-closure both resolve to not-found. A constraint refusal — `ErrBranchNotCreatable`, `ErrUnreadableReference`, `ErrReservedType` — resolves to forbidden: the contribution asked for more than its grants gave, which is the caller's answer rather than an internal fault. Caught by the smoke run, which had it as a 500.
...
- [x] 6.9 Answered in v0.12.0: `WithCreatableBranches` names the branches a contribution may bring into being, and `Archive.MissingBranches` reports which of the declared ones the base lacks. So the matching is this server's, as it should be — it asks what would be created, gates that on C over the branch table, and passes the permitted list. `Narrow` then does the rest: `Creatable` empty asks for nothing, so a client that does not declare an intent to create gets none however broad its grants.
- [x] 6.3 Answered in v0.11.0: `WithReferencableBranches` bounds what a contribution may reference, and `Constraints.AdmitReferences` enforces step 3 at completion. Nothing about users crosses — the server composes what the request asked for with what the account's grants allow, and `Narrow` takes the overlap, so a client can only ever tighten its own reach.
...
- [x] 6.4 Fixed upstream in v0.10.0: `admissible()` checked only `NodeBranches`, and now defers to `Constraints.AdmitType`, whose reserved set carries the limiting claims too (`NodeDelete`, `NodeExpiry`). A contribution opens under the strictest reading, which is what a client contribution should get, so none of the three can be minted from a request — pinned by a test here. The other half of the finding — step 6's `expires_after_request` → `contribution/expiry` mint, absent from `concurrent`, leaving `core-limiting-claims` unsatisfiable by that backend — is **deferred by decision**, not overlooked: deletion and expiry are their own subject and are taken up separately.
...
- [x] 7.3 Wired: the principal's R grants become the referencable scopes (`$universe` when held, else each readable branch), `MissingBranches` against the declared set becomes the creatable list gated on C over the branch table, and C on `$branches` lifts the branch-table type. Each is narrowed against what the body asked for, and the Sequencer enforces the result.
