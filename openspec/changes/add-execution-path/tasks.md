...
- [ ] 6.3 Ask ranke-go for a read check on the completion walk, as an option in the shape `WithTrusted` already uses — a caller-supplied predicate, called per claim the walk draws in from outside the contribution, given the claim's id and the branch it was found on (empty when no branch holds it). Refusing fails the contribution. `CompleteAndVerify(ctx)` takes no options today, so steps 3–4 traverse with no seam. The predicate is all this repo needs: paper 02 §Sequencer step 3 requires read access to every branch-external claim, and the paper's own wording — drawn in "from another branch of the archive or the wider Universe" — fixes what to check, so `core-access` answers it with R on the naming branch, or `R $universe` where no branch holds the claim. Nothing about the policy is unsettled; only the hook is missing.
...
- [ ] 7.3 Wire the read check from 6.3 into the closure walk: answer the predicate from `core-access` — R on the branch the claim was found on, or `R $universe` when none holds it. Blocked on the hook, and deliberately not approximated here: the walk is recursive, so a check over only the references this repo can see would leave the transitive tail unenforced while reading as though it were covered.
