...
- [ ] 1.1 `parseAddr` reads `unix://` and returns the network and the path, refusing
      `unix://` with no path. Anything without the scheme is a TCP address as before.
- [ ] 1.2 `net.Listen` on the parsed network followed by `srv.Serve(ln)` in place of
...
- [ ] 1.3 Test both forms bind and serve a real request over the listener they claim, the TCP
...
- [ ] 4.1 Check the address form in `verify`: a `unix://` path within the platform's
...
- [ ] 4.2 Report a filesystem path carrying no scheme as a malformed address, naming the
...
- [ ] 5.1 Add a second endpoint to `examples/minimal/config.json`: `unix://` under a
