...
# dependency (direct + indirect + test), the tool dependencies, and ranke-go (the
# upstream library). Then tidy and run the green gate, so a breaking upgrade shows
# up here instead of in the next build.
#
...
#
# ranke-go is fetched LAST: `go get -u` has already moved it to the latest
# minor/patch, so the final fetch is what makes an explicit pin stick.
#
...
# A tool dependency is imported by no package, so ./... never reaches it. No -u
# here: a tool is a program, so we want its latest RELEASE with the dependency
# versions its own go.mod asks for. `-u` drags a tool's untagged deps to tip
# (go-yit, say) and breaks generation for no gain.
if grep -q '^tool ' go.mod; then
	echo ">> tools: go get tool"
	go get tool
	hold_go_directive
fi

if grep -q "$RANKE_GO_MOD" go.mod; then
	echo ">> $RANKE_GO_MOD: fetching $RANKE_GO_VERSION"
	go get "$RANKE_GO_MOD@$RANKE_GO_VERSION"
	hold_go_directive
fi
...
# Every changed require/go/tool line, without diff's file headers or hunk marks.
echo ">> go.mod changes:"
diff -u "$before" go.mod | grep -E '^[-+][^-+]' | sed 's/^/     /'

echo ">> verifying the upgrade"
if "${MAKE:-make}" -s verify; then
