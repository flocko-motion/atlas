...
Each claim names its own branch, so **one contribution may advance several**; the
**C** right is required on every branch the body writes to. Send every claim the
closure needs that the archive may not hold yet — a first-time contributor
includes its own `contribution/contributor` claim, since everything it signs
references it.
