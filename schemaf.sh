...
case "$CMD" in
  ...
  test)
    ...
    docker compose -f compose.test.gen.yml up -d --wait

    # Query the random host port Docker assigned to postgres:5432.
    PG_PORT=$(docker compose -f compose.test.gen.yml port postgres 5432 | awk -F: '{print $NF}')
    ...
    TEST_EXIT=$?
    docker compose -f compose.test.gen.yml down
    exit $TEST_EXIT
    ;;
  run)
    if [ ! -f compose.gen.yml ]; then
